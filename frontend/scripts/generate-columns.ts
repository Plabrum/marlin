...
interface ResolvedStateMachine extends StateMachineMeta {
  resource: string;
}
...
interface CrudMetadata {
  [operationId: string]: {
    ...
    state_machine?: StateMachineMeta;
  };
}
...
async function main() {
  ...
  let generated = 0;
  const resolvedStateMachines: ResolvedStateMachine[] = [];
  ...
  for (const [operationId, meta] of Object.entries(metadata)) {
    ...
    const stateColumn = meta.state_machine?.column ?? null;

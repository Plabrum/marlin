...
export function SurveyWorkspace({ data }: { data: SurveyDetail }) {
  const { sections, completion, findings, findingsByParent, sectionAncestor } =
    useSurveyTree(data.form_nodes, data.section_completion);
  ...
  return (
    <div className="bg-background relative h-[calc(100vh-3.5rem)]">
      ...
      <div className="grid h-full grid-cols-1 gap-0 md:grid-cols-[minmax(0,1fr)_27rem]">
            ...
            {currentSection && (
              <SectionBlock
                ...
                previousSectionCompletion={
                  previousSection
                    ? completion.get(previousSection.id)
                    : undefined
                }
        ...
        <aside className="bg-sidebar/60 md:border-sidebar-border hidden h-full overflow-y-auto px-4 py-6 [scrollbar-width:none] md:block md:border-l md:px-6 md:py-8 [&::-webkit-scrollbar]:hidden">
          <PhotosRail
            surveyId={data.id}

...
const TONE_CLASSES: Record<NonNullable<ErrorStateProps['iconTone']>, string> = {
  ...
  warning:
    'bg-[var(--status-warning)]/10 text-[var(--status-warning)] ring-[var(--status-warning)]/30',
  danger:
    'bg-[var(--status-danger)]/10 text-[var(--status-danger)] ring-[var(--status-danger)]/30',
};
...
export function ErrorState({
  ...
  return (
    <div className="flex flex-1 items-center justify-center px-4 py-12">
      <div className="border-border bg-card w-full max-w-sm rounded-2xl border p-10 shadow-sm">
        <div className="space-y-5 text-center">
          ...
          <div>
            <h1 className="font-display text-foreground mb-2 text-lg font-normal">
              {title}
            </h1>

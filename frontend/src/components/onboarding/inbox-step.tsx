...
export function InboxStep() {
  ...
  const execute = useActionsActionGroupExecuteAction({
    mutation: {
      ...
      onError: (err) =>
        toast.error(getErrorMessage(err, 'Could not claim inbox')),
    },
  });
...
function renderStatus(
  ...
  isFetching: boolean,
  availability:
    | { available: boolean; canonical: string; reason?: string | null }
    | undefined
): { message: string; tone: string } | null {
  if (!value) return null;
  if (isFetching)
    return { message: 'Checking…', tone: 'text-sm text-muted-foreground' };
